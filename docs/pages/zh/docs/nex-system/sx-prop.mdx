...
而 CSS 属性值可以直接使用预定义的主题。如果您不满足于已有的 CSS 自定义属性和选择器以及主题，Nex System 还提供了扩展能力。在<Link href="">`Theming`页面</Link>了解更多信息。
...
export default function Example() {
  return (
    <NexProvider theme={theme}>
      <Box
        sx={{
          // 只有在 scales 中定义 color: 'colors', color 可以使用 token.colors.primary
          color: 'primary',

...
Semantic Tokens 是专为特定上下文使用而设计的 Tokens。与 Tokens 不同，其更侧重于传达内容的语义信息。
...
Semantic Tokens 可以嵌套，以创建层次结构。这在将 Semantic Tokens 组合在一起时，非常有用。

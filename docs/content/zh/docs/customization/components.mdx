...
`styleOverrides` 支持两种不同方式覆盖组件的样式：函数或者对象（[Recipes]()、[SlotRecipes]()）。

### 函数方式

使用回调函数来访问组件的 `ownerState`，返回 [`sx`]()。
...
使用 [Recipes]() 或者 [SlotRecipes]() （由不同的组件决定）不仅可以覆盖组件的默认样式，还包括各种不同 `variants` 以及组合式 `variants`的样式。
...
`styleOverrides` 不仅可以覆盖组件样式，还可以去扩展组件样式。
...
如果你正在使用 TypeScript，就需要指定新的 `size` 。

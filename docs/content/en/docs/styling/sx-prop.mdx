...
The basic requirement for using `colorPalette` is that the colors used must have a consistent naming convention. By default, NexUI provides color values from 50 to 900 for each color. This makes it easier for you to create and use `colorPalette`.

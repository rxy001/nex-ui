...
> Nex UI uses a mobile-first breakpoint system. Responsive styles are generated with min-width media queries (`@media (min-width)`).

Nex UI provides five default breakpoints:
...
There are two ways to define responsive values: object syntax and array syntax.
...
The second approach is to define values as an array, ordered from small to large breakpoints:
...
<Callout type='info'>
Use this approach only when your theme has a small number of breakpoints (for example, three).
...
Whether you use an array or an object, values like `width` can reference size tokens or semantic size tokens. The same principle applies to other CSS properties and their corresponding token types.

## Breakpoints Selector

The two methods above are great for responsive values on a single CSS property. If you need responsive behavior for a group of properties, use breakpoint selectors:
...
This approach is often easier to read and maintain in complex styling scenarios.

...
description: Semantic tokens are designed for specific contexts. Unlike raw tokens, they focus on conveying semantic meaning.
---
...
Semantic tokens are designed for specific contexts. Unlike raw tokens, they focus on conveying semantic meaning.
...
If your project uses TypeScript, you can extend custom semantic tokens through [Module Augmentation](https://www.typescriptlang.org/docs/handbook/declaration-merging.html#module-augmentation).
...
Semantic tokens can also change based on conditions such as light or dark mode. Nex UI's dark theme is implemented this way.
...
Semantic tokens can be nested to create a hierarchical structure, which is useful when composing semantic values.
...
**You can also use conditions like `_dark` in nested structures.**

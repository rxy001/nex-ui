...
import { useCollectionRootContext } from './CollectionContext'
...
export const useCollectionItem = createHook<
  ...
  CollectionItemState
>(function useCollectionItem({ getItem, ...props }: UseCollectionItemProps) {
  const { addItem, removeItem } = useCollectionRootContext()
